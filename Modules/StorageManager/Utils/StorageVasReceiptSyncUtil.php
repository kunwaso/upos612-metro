...
class StorageVasReceiptSyncUtil
{
    ...
    {
        return $this->unlinkDocumentVasSync($businessId, $receipt, $userId);
    }

    public function unlinkDocumentVasSync(int $businessId, StorageDocument $document, int $userId): array
    {
        if (! in_array((string) $document->document_type, ['receipt', 'putaway'], true)) {
            throw new RuntimeException('Only receipt or putaway documents can be unlinked from VAS accounting.');
        }

        if (! in_array((string) $document->status, ['completed', 'closed'], true)) {
            throw new RuntimeException('Storage document must be completed or closed before unlinking accounting.');
        }

        $actionTaken = 'none';
        $vasDocumentId = $document->vas_inventory_document_id ? (int) $document->vas_inventory_document_id : null;
        ...
        } elseif ($vasDocumentId) {
            $actionTaken = 'detached_without_vas_tables';
        }
