...
@section('content')
    @php
        $document = $voucher ?? $paymentDocument ?? null;
        ...
        $selectedTargets = collect(old('settlement_targets', data_get($prefill, 'settlement_targets', [])))->keyBy('target_voucher_id');
                                    ...
                                    <table class="table align-middle table-row-dashed fs-7 gy-3">
                                        ...
                                        <tbody id="payable-targets" {{ $isReceipt ? 'style=display:none;' : '' }}>
                                            @forelse ($payableItems as $item)
                                                @php($target = $selectedTargets->get($item->id))
                                                <tr>
                                                    <td>
                                                        <input type="checkbox" class="form-check-input settlement-toggle" data-row="payable-edit-{{ $loop->index }}" {{ $target ? 'checked' : '' }}>
                                                    </td>
                                                    ...
                                                    <td class="text-end">
                                                        ...
                                                        <input type="number" min="0.0001" step="0.0001" class="form-control form-control-solid text-end settlement-amount" id="payable-edit-{{ $loop->index }}" name="settlement_targets[{{ $loop->index }}][amount]" value="{{ data_get($target, 'amount') }}" {{ $target ? '' : 'disabled' }}>
                                                    </td>
                                                </tr>
                                            @empty
                                        ...
                                        <tbody id="receivable-targets" {{ $isReceipt ? '' : 'style=display:none;' }}>
                                            @forelse ($receivableItems as $item)
                                                ...
                                                <tr>
                                                    <td>
                                                        <input type="checkbox" class="form-check-input settlement-toggle" data-row="receivable-edit-{{ $loop->index }}" {{ $target ? 'checked' : '' }}>
                                                    </td>
                                                    <td>{{ $item->voucher_no }}</td>
                                                    <td>{{ $item->contact_name }}</td>
                                                    <td class="text-end">{{ number_format((float) $item->outstanding_amount, 2) }}</td>
                                                    <td class="text-end">
                                                        ...
                                                        <input type="number" min="0.0001" step="0.0001" class="form-control form-control-solid text-end settlement-amount" id="receivable-edit-{{ $loop->index }}" name="settlement_targets[{{ count($payableItems) + $loop->index }}][amount]" value="{{ data_get($target, 'amount') }}" {{ $target ? '' : 'disabled' }}>
                                                    </td>
                                                </tr>
                                            @empty
