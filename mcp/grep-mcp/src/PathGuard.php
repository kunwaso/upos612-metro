...
final class PathGuard
{
    ...
    public function __construct(string $workspaceRoot)
    {
        ...
        if ($resolved === false || !is_dir($resolved)) {
            throw new GrepException('WORKSPACE_ROOT_INVALID', 'Workspace root is invalid.');
        }
    ...
    public function assertSearchPath(string $path): string
    {
        ...
        if (!file_exists($resolved)) {
            throw new GrepException('SEARCH_PATH_NOT_FOUND', 'Search path not found.', $originalPath);
        }

        $canonical = realpath($resolved);
        if ($canonical === false) {
            throw new GrepException('SEARCH_FAILED', 'Unable to resolve search path.', $originalPath);
        }
    ...
    private function normalizeInputPath(string $path): string
    {
        $path = trim($path);
        if ($path === '') {
            throw new GrepException('INVALID_ARGUMENT', 'Path is required.');
        }
    ...
    private function assertWithinWorkspace(string $resolvedPath, string $originalPath): void
    {
        if (!$this->isWithin($resolvedPath, $this->workspaceRoot)) {
            throw new GrepException('PATH_NOT_ALLOWED', 'Searching this path is not allowed.', $originalPath);
        }
    }
