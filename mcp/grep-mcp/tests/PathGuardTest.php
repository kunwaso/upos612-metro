...
final class PathGuardTest extends TestCase
{
    ...
    public function test_it_resolves_relative_absolute_and_mixed_separator_paths_inside_workspace(): void
    {
        ...
        self::assertSame($expected, $this->normalizePath($this->guard->assertSearchPath('notes/example.txt')));
        self::assertSame($expected, $this->normalizePath($this->guard->assertSearchPath($expected)));
        self::assertSame(
            $expected,
            $this->normalizePath($this->guard->assertSearchPath(str_replace('/', '\\', $expected)))
        );
    ...
    {
        $this->assertGrepException('PATH_NOT_ALLOWED', $path, function () use ($path): void {
            $this->guard->assertSearchPath($path);
        });
    }

    public function test_it_rejects_missing_and_outside_paths_and_filters_non_throwing_checks(): void
    {
        $outside = dirname($this->workspaceRoot()).'/outside.txt';
        file_put_contents($outside, "nope\n");

        try {
            $this->assertGrepException('SEARCH_PATH_NOT_FOUND', 'notes/missing.txt', function (): void {
                $this->guard->assertSearchPath('notes/missing.txt');
            });
            $this->assertGrepException('PATH_NOT_ALLOWED', $outside, function () use ($outside): void {
                $this->guard->assertSearchPath($outside);
            });
            $this->assertGrepException('PATH_NOT_ALLOWED', '../outside.txt', function (): void {
                $this->guard->assertSearchPath('../outside.txt');
            });

            self::assertTrue($this->guard->isPathAllowed('notes/example.txt'));
            self::assertFalse($this->guard->isPathAllowed('.env'));
            self::assertFalse($this->guard->isPathAllowed('../outside.txt'));
        } finally {
            @unlink($outside);
        }
    }
    ...
    public function test_it_accepts_drive_letter_case_differences_on_case_insensitive_platforms(): void
    {
        ...
        self::assertSame($expected, $this->normalizePath($this->guard->assertSearchPath($variant)));
    }
     ...
     */
    private function assertGrepException(string $expectedCode, ?string $expectedPath, callable $callback): void
    {
        try {
            $callback();
            self::fail('Expected GrepException was not thrown.');
