...
4. `semantic_code_search` - optional

Run `php scripts/check-mcp-health.php` from repo root to verify the current machine is ready. The health check uses path-scoped probes for grep, validates that `read_file_cache` returns actual file text, and reports semantic readiness as `READY`, `NOT_INDEXED`, `STALE`, or `OLLAMA_UNAVAILABLE`.
