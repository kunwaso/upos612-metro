<div class="modal-dialog modal-xl" role="document">
	<div class="modal-content">
		...
		<div class="modal-body">
		  	<div class="row">
			    <div class="col-sm-12">
			      <p class="pull-right"><b>@lang('messages.date'):</b> {{ format_date_value($stock_adjustment->transaction_date) }}</p>
			    </div>
			</div>
			<div class="row invoice-info">
			    ...
			    <div class="col-sm-4 invoice-col">
			      	...
			      	<b>@lang('messages.date'):</b> {{ format_date_value($stock_adjustment->transaction_date) }}<br/>
								...
								<tr>
									...
									@if(!empty($lot_n_exp_enabled))
						                <td>{{ $stock_adjustment_line->lot_details->lot_number ?? '--' }}
						                    ...
						                    ({{ format_date_value($stock_adjustment_line->lot_details->exp_date) }})
						                    @endif
						                </td>
						            @endif
									<td>
										{{ format_quantity_value($stock_adjustment_line->quantity) }}
									</td>
									<td class="@cannot('view_purchase_price') show_price_with_permission no-print @endcan">
										{{ num_format_value($stock_adjustment_line->unit_price) }}
									</td>
									<td class="@cannot('view_purchase_price') show_price_with_permission no-print @endcan">
										{{ num_format_value($stock_adjustment_line->unit_price * $stock_adjustment_line->quantity) }}
									</td>
								</tr>
							@endforeach
						</table>
