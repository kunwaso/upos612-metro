<div class="row">
  <div class="col-xs-12">
    <h2 class="page-header">
      ...
      <small class="pull-right"><b>@lang('messages.date'):</b> {{ format_date_value($sell_transfer->transaction_date) }}</small>
    </h2>
  </div>
</div>
<div class="row invoice-info">
  ...
  <div class="col-sm-4 invoice-col">
    ...
    <b>@lang('messages.date'):</b> {{ format_date_value($sell_transfer->transaction_date) }}<br/>
  </div>
</div>
              ...
              {{ $sell_lines->product->name }}
               ...
               @if($lot_n_exp_enabled && !empty($sell_lines->lot_details))
                ...
                @if(!empty($sell_lines->lot_details->exp_date))
                  - {{ format_date_value($sell_lines->lot_details->exp_date) }}
                @endif
               @endif
            </td>
            <td>{{ format_quantity_value($sell_lines->quantity) }} {{$sell_lines->product->unit->short_name ?? ""}}</td>
