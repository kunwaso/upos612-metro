    ...
    <div class="d-flex flex-column flex-root min-vh-100 align-items-center justify-content-center p-4">
        <div class="card card-flush w-100" style="max-width: 460px;">
            <div class="card-body p-10">
                <h3 class="fw-bold mb-3">{{ __('product.public_link_password') }}</h3>
                <p class="text-muted mb-6">{{ __('product.enter_password_to_view_quote') }}</p>

                @if($isLocked)
                    <div class="alert alert-danger mb-6">{{ $lockedMessage ?? __('product.quote_unlock_blocked_10min') }}</div>
                @endif
                ...
                <form method="POST" action="{{ route('product.quotes.public.unlock', ['publicToken' => $publicToken]) }}">
                    @csrf
