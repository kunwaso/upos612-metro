...
<div class="timeline">
    @forelse($logs as $log)
        @php
            ...
            $userName = __('product.system_user');
                ...
                <div class="pe-3 mb-5">
                    <div class="d-flex align-items-start justify-content-between gap-3">
                        ...
                        @if($canDeleteActivity)
                            <form method="POST" action="{{ route('product.detail.activity.delete', ['id' => $product->id, 'log_id' => $log->id]) }}" onsubmit="return confirm('{{ __('product.activity_delete') }}?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-icon btn-sm btn-light" title="{{ __('product.activity_delete') }}">
                    ...
                    <div class="d-flex align-items-center mt-1 fs-6">
                        <div class="text-muted me-2 fs-7">{{ __('product.activity_added_at') }} {{ optional($log->created_at)->format('M j, Y g:i A') }} {{ __('product.activity_by') }}</div>
    ...
    @empty
        <div class="timeline-item">
            ...
            <div class="timeline-content mt-n1">
                <div class="pe-3 mb-5">
                    <div class="fs-5 fw-semibold mb-2">{{ __('product.activity_no_data_period') }}</div>
                </div>
            </div>
        </div>
    @endforelse
