...
@section('title', __('product.sales_orders'))
...
<div class="d-flex flex-wrap flex-stack mb-6">
    <div>
        <h1 class="text-gray-900 fw-bold mb-1">{{ __('product.sales_orders') }}</h1>
        <div class="text-muted fw-semibold fs-6">{{ __('product.sales_orders_description') }}</div>
    </div>
    <a href="{{ route('product.quotes.index') }}" class="btn btn-light-primary btn-sm">
        ...
        {{ __('product.product_quotes') }}
    </a>
</div>

<div class="card card-flush">
    <div class="card-header pt-7">
        <h3 class="card-title fw-bold text-gray-900">{{ __('product.sales_orders') }}</h3>
    </div>
            ...
            <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4" id="projectx_orders_table">
                <thead>
                    <tr class="fw-bold text-muted text-uppercase fs-7">
                        ...
                        <th>{{ __('product.payment_status') }}</th>
...
<script>
    (function () {
        ...
        table.DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('product.sales.orders.index') }}',
            order: [[5, 'desc']],
            columns: [
        ...
        $(document).on('click', '.js-toggle-order-hold', function () {
            ...
            $.ajax({
                method: 'PATCH',
                url: '{{ route('product.sales.orders.hold.update', ['id' => '__ID__']) }}'.replace('__ID__', orderId),
                data: {
                    _token: '{{ csrf_token() }}',
