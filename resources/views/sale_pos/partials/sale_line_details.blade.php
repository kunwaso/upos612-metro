        ...
        <tr>
            ...
            <td>
                ...
                @if($is_warranty_enabled && !empty($sell_line->warranties->first()) )
                    <br><small>{{$sell_line->warranties->first()->display_name ?? ''}} - {{ format_date_value($sell_line->warranties->first()->getEndDate($sell->transaction_date)) }}</small>
            ...
            @if( session()->get('business.enable_lot_number') == 1 && empty($for_ledger))
                <td>{{ $sell_line->lot_details->lot_number ?? '--' }}
                    ...
                    ({{ format_date_value($sell_line->lot_details->exp_date) }})
                    @endif
                </td>
            @endif
            @if($sell->type == 'sales_order')
            ...
            <td>
                @if(!empty($for_ledger))
                    {{ format_quantity_value($sell_line->quantity) }}
                @else
                ...
                @if(!empty($sell_line->product->second_unit) && $sell_line->secondary_unit_quantity != 0)
                    <br>
                    @if(!empty($for_ledger))
                        {{ format_quantity_value($sell_line->secondary_unit_quantity) }}
                    @else
