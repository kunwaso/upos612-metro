						...
						<td>
							...
							@if(!Gate::check('disable_express_checkout') || auth()->user()->can('superadmin'))
								<span class="@if(!$is_discount_enabled && !$is_rp_enabled) hide @endif">
									...
									<input type="hidden" name="discount_amount" id="discount_amount" value="@if(empty($edit)) {{ num_format_value($business_details->default_sales_discount) }} @else {{ num_format_value($transaction->discount_amount) }} @endif" data-default="{{$business_details->default_sales_discount}}">
							...
							<div class="{{$col}} col-xs-6 d-inline-table">
								...
								<input type="hidden" name="shipping_charges" id="shipping_charges" value="@if(empty($edit)){{ num_format_value(0.00) }} @else{{ num_format_value($transaction->shipping_charges) }} @endif" data-default="0.00">
