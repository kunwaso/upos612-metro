...
<br>
		<small class="text-muted p-1">
			...
			{{ num_format_value($product->qty_available) }} {{$product->unit}} @lang('lang_v1.in_stock')
			@else
				--
			@endif
		</small>
	...
	<td>
		...
		<input type="text" data-min="1" style="width: auto"
			...
			value="{{ format_quantity_value($product->quantity_ordered) }}" name="products[{{$row_count}}][quantity]" data-allow-overselling="@if(empty($pos_settings['allow_overselling'])){{'false'}}@else{{'true'}}@endif" 
			@if($allow_decimal) 
				data-decimal=1 
			@else 
				data-decimal=0 
		...
		@if(!empty($product->second_unit))
            ...
            value="{{ format_quantity_value($product->secondary_unit_quantity) }}"
	...
	@if(!empty($is_direct_sell))
		...
		<td class="@if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
			<input type="text" name="products[{{$row_count}}][unit_price]" class="form-control pos_unit_price input_number mousetrap" value="{{ num_format_value($pos_unit_price) }}" @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$pos_unit_price}}" data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => num_format_value($pos_unit_price)])}}" @endif> 
		...
		<td @if(!$edit_discount) class="hide" @endif>
			{!! Form::text("products[$row_count][line_discount_amount]", num_format_value($discount_amount), ['class' => 'form-control input_number row_discount_amount']); !!}<br>
			...
			@if(!empty($last_sell_line))
				...
				<small class="text-muted">
					...
					@if($last_sell_line->line_discount_type == 'percentage')
						{{ num_format_value($last_sell_line->line_discount_amount) }}%
					@else
		...
		<td class="text-center {{$hide_tax}}">
			{!! Form::hidden("products[$row_count][item_tax]", num_format_value($item_tax), ['class' => 'item_tax']); !!}
	...
	@endif
	<td class="{{$hide_tax}}">
		<input type="text" style="width: auto" name="products[{{$row_count}}][unit_price_inc_tax]" class="form-control pos_unit_price_inc_tax input_number" value="{{ num_format_value($unit_price_inc_tax) }}" @if(!$edit_price) readonly @endif @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$unit_price_inc_tax}}" data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => num_format_value($unit_price_inc_tax)])}}" @endif>
	</td>
	...
	<td class="text-center">
		...
		@endphp
		<input style="width: auto" type="{{$subtotal_type}}" class="form-control pos_line_total @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="{{ num_format_value($product->quantity_ordered*$unit_price_inc_tax) }}">
