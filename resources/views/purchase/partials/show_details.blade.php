...
<div class="modal-body">
  <div class="row">
    <div class="col-sm-12">
      <p class="pull-right"><b>@lang('messages.date'):</b> {{ format_date_value($purchase->transaction_date) }}</p>
    </div>
  </div>
  <div class="row invoice-info">
    ...
    <div class="col-sm-4 invoice-col">
      ...
      <b>@lang('messages.date'):</b> {{ format_date_value($purchase->transaction_date) }}<br/>
            ...
            <tr>
              ...
              <td>
                @if(!empty($purchase_line->mfg_date))
                    {{ format_date_value($purchase_line->mfg_date) }}
                @endif
              </td>
              <td>
                @if(!empty($purchase_line->exp_date))
                    {{ format_date_value($purchase_line->exp_date) }}
                @endif
              </td>
              @endif
              @endif
      ...
      <div class="table-responsive">
        <table class="table">
          ...
          @forelse($purchase->payment_lines as $payment_line)
            ...
            <tr>
              <td>{{ $loop->iteration }}</td>
              <td>{{ format_date_value($payment_line->paid_on) }}</td>
