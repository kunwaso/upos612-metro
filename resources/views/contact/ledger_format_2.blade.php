		...
		<tbody>
			@foreach($ledger_details['ledger'] as $data)
				...
				<tr @if(!empty($for_pdf) && $loop->iteration % 2 == 0) class="odd" @endif style="border:hidden;">
					<td class="row-border">{{ format_datetime_value($data['date']) }}</td>
					<td>@if($loop->index == 0) {{$data['type']}} @endif {{$data['ref_no']}} @if(!empty($data['due_date']) && $data['payment_status'] != 'paid') <br>@lang('lang_v1.due') {{ format_date_value($data['due_date']) }} @endif</td>
