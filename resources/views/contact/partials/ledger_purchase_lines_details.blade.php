<table class="table table-slim mb-0 bg-light-gray" @if(!empty($for_pdf)) style="width: 100%;" @endif>
  ...
  @foreach($purchase->purchase_lines as $purchase_line)
    <tr>
      ...
      <td>{{ format_quantity_value($purchase_line->quantity) }} @if(!empty($purchase_line->sub_unit)) {{$purchase_line->sub_unit->short_name}} @else {{$purchase_line->product->unit->short_name}} @endif
      </td>
      <td >@format_currency($purchase_line->pp_without_discount)</td>
      <td >{{ num_format_value($purchase_line->discount_percent) }} %</td>
