	...
	<table class="tpdf">
		<tr>
			<td class="width-50">
				...
				<strong>@lang('lang_v1.order_date'):</strong> {{ format_date_value($purchase->transaction_date) }}
			</td>
			<td class="width-50">
				{{-- <strong>Due date:</strong> {{ format_date_value($purchase->due_date) }}<br> --}}
				...
				@if(!empty($purchase->delivery_date))
					{{ format_date_value($purchase->delivery_date) }}
				@else
					{{'-'}}
				@endif
			</td>
	...
	<table class="table-pdf td-border">
		...
		@foreach($purchase->purchase_lines as $purchase_line)
			<tr @if($loop->iteration % 2 !== 0) class="odd" @endif style="border:hidden;">
				...
				@endif
				<td>
					{{ format_quantity_value($purchase_line->quantity) }} @if(!empty($purchase_line->sub_unit)) {{$purchase_line->sub_unit->actual_name}}  @else {{$purchase_line->product->unit->actual_name}} @endif   {{-- Display the base_unit_multiplier here --}}
