...
@foreach($products as $product)
	...
	<tr data-variation_id="{{$product->variation_id}}">
		<td>
			...
			@endif
			<p class="help-block">@lang('report.current_stock'): {{ format_quantity_value($product->stock) }} {{$product->unit}}</p>
		</td>
		<td>{{ format_quantity_value($product->alert_quantity) }} {{$product->unit}}</td>
		<td>
