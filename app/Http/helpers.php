...
if (! function_exists('num_format_value')) {
    /**
     * Format a number for currency display (same logic as @num_format Blade directive).
     * Returns a string suitable for display or form value.
     *
     * @param  float|int|string  $number
     * @return string
     */
    function num_format_value($number)
    {
...
if (! function_exists('format_quantity_value')) {
    /**
     * Format a number as quantity (same logic as @format_quantity Blade directive).
     * Returns a string suitable for display or form value.
     *
     * @param  float|int|string  $number
     * @return string
     */
    function format_quantity_value($number)
    {
        $currency = session('currency', []);
        $decimal_separator = $currency['decimal_separator'] ?? '.';
        $thousand_separator = $currency['thousand_separator'] ?? ',';
...
if (! function_exists('format_date_value')) {
    /**
     * Format a date for display (same logic as @format_date Blade directive).
     *
     * @param  mixed  $date
     * @return string|null
     */
    function format_date_value($date)
    {
        ...
        return \Carbon\Carbon::createFromTimestamp(strtotime($date))->format(session('business.date_format', 'm/d/Y'));
    }
}

if (! function_exists('format_datetime_value')) {
    /**
     * Format a date-time for display (same logic as @format_datetime Blade directive).
     *
     * @param  mixed  $date
     * @return string|null
     */
    function format_datetime_value($date)
    {
        ...
        return \Carbon\Carbon::createFromTimestamp(strtotime($date))->format(session('business.date_format', 'm/d/Y').' '.$time_format);
    }
}

if (! function_exists('format_time_value')) {
    /**
     * Format time for display (same logic as @format_time Blade directive).
     *
     * @param  mixed  $date
     * @return string|null
     */
    function format_time_value($date)
    {
        if (empty($date)) {
            return null;
        }
