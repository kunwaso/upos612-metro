...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getRegister()
    {
        ...
        $registration_language = request()->query('lang', config('app.locale'));
        $default_fy_start_month = (string) old('fy_start_month', date('n'));
        $default_accounting_method = (string) old('accounting_method', 'fifo');
        if (! array_key_exists($default_accounting_method, $accounting_methods)) {
            $default_accounting_method = (string) (array_key_first($accounting_methods) ?? '');
        }
