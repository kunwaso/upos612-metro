...
class HomeController extends Controller
{
    ...
    protected $homeMetronicDashboardUtil;
    ...
    public function __construct(
        ...
        HomeMetronicDashboardUtil $homeMetronicDashboardUtil
    ) {
        ...
        $this->homeMetronicDashboardUtil = $homeMetronicDashboardUtil;
    }
    ...
    public function index()
    {
        ...
        $dashboardData = $this->homeMetronicDashboardUtil->emptyPayload();
        $dashboardMeta = data_get($dashboardData, 'meta', []);
        $dashboardKpis = data_get($dashboardData, 'kpis', []);
        $dashboardCharts = data_get($dashboardData, 'charts', []);
        $dashboardRecentTabs = data_get($dashboardData, 'recent_orders_tabs', []);
        $dashboardProductOrders = data_get($dashboardData, 'product_orders', []);
        $dashboardDeliveryFeed = data_get($dashboardData, 'delivery_feed', []);
        $dashboardStockRows = data_get($dashboardData, 'stock_rows', []);
        ...
        if (! auth()->user()->can('dashboard.data')) {
            ...
            return view('home.index', compact(
                ...
                'dashboardDeliveryFeed',
                'dashboardStockRows'
            ));
        }

        $fy = request()->session()->get('financial_year') ?? $this->businessUtil->getCurrentFinancialYear($business_id, $business);

        $currencyData = request()->session()->get('currency');
        $currency = isset($currencyData['code'])
            ? (object) $currencyData
        ...
        try {
            $dashboardData = $this->homeMetronicDashboardUtil->getDashboardData($business_id, null, ['range' => 'month']);
        } catch (\Throwable $exception) {
            \Log::error('Initial Metronic dashboard payload failed: '.$exception->getMessage(), [
    ...
    public function getMetronicDashboardData(Request $request)
    {
        $business_id = $request->session()->get('user.business_id');
        if (empty($business_id) || ! auth()->user()->can('dashboard.data')) {
            return response()->json($this->homeMetronicDashboardUtil->emptyPayload());
        }
        ...
        $sales_chart_filter = [
            'range' => (string) $request->input('sales_chart_range', 'month'),
            'start_date' => $request->input('sales_chart_start_date'),
            'end_date' => $request->input('sales_chart_end_date'),
        ];

        try {
            $payload = $this->homeMetronicDashboardUtil->getDashboardData($business_id, $location_id, $sales_chart_filter);

            return response()->json($payload);
        } catch (\Throwable $exception) {
            \Log::error('Metronic dashboard payload failed: '.$exception->getMessage(), [
                'file' => $exception->getFile(),
                'line' => $exception->getLine(),
                'business_id' => $business_id,
            ]);
