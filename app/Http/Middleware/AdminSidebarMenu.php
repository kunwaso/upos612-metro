            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        if (auth()->user()->can('product_quote.view') || auth()->user()->can('product_quote.create')) {
                            $sub->url(
                                action([\App\Http\Controllers\ProductQuoteController::class, 'index']),
                                __('product.quote_management'),
                                ['icon' => '', 'active' => request()->segment(1) == 'product' && request()->segment(2) == 'quotes']
                            );
                        }

                        if ($is_admin || auth()->user()->can('sell.view') || auth()->user()->can('direct_sell.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ProductSalesOrderController::class, 'index']),
                                __('product.view_sales_orders'),
                                ['icon' => '', 'active' => request()->segment(1) == 'product' && request()->segment(2) == 'sales' && request()->segment(3) == 'orders']
                            );
                        }
