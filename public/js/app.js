$(document).ready(function() {
    ...
    var contact_table_page_length = parseInt(__default_datatable_page_entries, 10);
    if (isNaN(contact_table_page_length) || contact_table_page_length <= 0) {
        contact_table_page_length = 25;
    }
    ...
    contact_table = $('#contact_table').DataTable({
        ...
        fixedHeader: false,
        autoWidth: false,
        pageLength: contact_table_page_length,
        lengthChange: false,
        scrollX: true,
        scrollCollapse: false,
        dom:
            "<'row'<'col-sm-12'tr>>" +
            "<'row align-items-center mt-5'<'col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start'i><'col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end'p>>",
        "ajax": {
            "url": "/contacts",
            "data": function ( d ) {
    ...
    $(document).on('input', '#contact_table_search', function() {
        if (typeof contact_table !== 'undefined') {
            contact_table.search($(this).val()).draw();
        }
    });

    $(document).on('change', '#has_sell_due, #has_sell_return, \
    ...
    $(document).on('click', '#contact_table_filters_reset', function() {
        var $filterPanel = $('#contact_table_filters_panel');

        $filterPanel.find('input[type="checkbox"]').prop('checked', false);
        $filterPanel.find('select').val('');

        contact_table.search('');
        $('#contact_table_search').val('');
        contact_table.ajax.reload();
    });
