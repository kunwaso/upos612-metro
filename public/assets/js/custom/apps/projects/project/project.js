...
var KTProjectOverview = function () {
    ...
    var normalizeChartPayload = function (payload) {
        var fallbackColors = [primary, success, danger, gray200];
        var chartPayload = payload || {};
        var labels = Array.isArray(chartPayload.labels) ? chartPayload.labels : [];
        var data = Array.isArray(chartPayload.data) ? chartPayload.data : [];
        var colors = Array.isArray(chartPayload.colors) ? chartPayload.colors : [];

        if (labels.length === 0) {
            return {
                labels: ['No components'],
                data: [0],
                colors: [gray200]
            };
        }
        ...
        return {
            labels: labels,
            data: labels.map(function (_, index) {
                var numericValue = parseFloat(data[index]);

                return isNaN(numericValue) ? 0 : numericValue;
            }),
            colors: labels.map(function (_, index) {
                return colors[index] || fallbackColors[index % fallbackColors.length];
            })
        };
    };

    var getChartPayloadFromElement = function (element) {
        try {
            return normalizeChartPayload(JSON.parse(element.getAttribute('data-chart') || '{}'));
        } catch (error) {
            return normalizeChartPayload({});
        }
    };

    var buildChartConfig = function (chartPayload) {
        return {
            type: 'doughnut',
            data: {
                datasets: [{
    ...
    var renderDoughnutChart = function (element, chartPayload) {
        if (doughnutChart) {
            doughnutChart.destroy();
        }

        var ctx = element.getContext('2d');
        doughnutChart = new Chart(ctx, buildChartConfig(chartPayload));
    };
    ...
    var initChart = function () {
        ...
        var initialPayload = getChartPayloadFromElement(element);
        renderDoughnutChart(element, initialPayload);

        window.KTProjectOverviewFabricComponents = {
            refreshChart: function (payload) {
                var normalizedPayload = normalizeChartPayload(payload);
                element.setAttribute('data-chart', JSON.stringify(normalizedPayload));
                renderDoughnutChart(element, normalizedPayload);
            }
        };
    }
