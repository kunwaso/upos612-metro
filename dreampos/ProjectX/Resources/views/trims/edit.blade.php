...
@section('title', __('projectx::lang.edit_trim'))

@section('content')
    <div class="d-flex flex-wrap flex-stack mb-6">
        <h2 class="fs-2 fw-semibold">{{ __('projectx::lang.edit_trim') }}</h2>
    ...
    @if(!empty($trim))
        @include('projectx::trims._trim_header', ['trim' => $trim, 'currency' => $currency ?? null, 'activeTab' => 'overview'])
    @endif

    <div class="card">
        <div class="card-header">
            <div class="card-title">
                <h3 class="fw-bold m-0">{{ __('projectx::lang.edit_trim') }}</h3>
            </div>
        </div>

        <form class="form" method="POST" action="{{ Route::has('projectx.trim_manager.update') && !empty($trim->id) ? route('projectx.trim_manager.update', ['id' => $trim->id]) : '#' }}" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <div class="card-body">
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-lg-9">
                        <input type="text" name="name" class="form-control form-control-solid" value="{{ old('name', $trim->name ?? '') }}" required />
                    </div>
                </div>
                                ...
                                <select name="trim_category_id" id="trim_category_select" class="form-select form-select-solid" data-control="select2">
                                    ...
                                    @foreach(($categories ?? []) as $categoryKey => $categoryItem)
                                        <option
                                            ...
                                            {{ (string) old('trim_category_id', $trim->trim_category_id ?? '') === (string) data_get($categoryItem, 'id', $categoryKey) ? 'selected' : '' }}
                                        >
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-lg-9">
                        <textarea name="description" class="form-control form-control-solid" rows="3">{{ old('description', $trim->description ?? '') }}</textarea>
                    </div>
                </div>
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-lg-9">
                        <select name="unit_of_measure" class="form-select form-select-solid" data-control="select2" data-hide-search="true">
                            <option value="pcs" {{ old('unit_of_measure', $trim->unit_of_measure ?? 'pcs') === 'pcs' ? 'selected' : '' }}>{{ __('projectx::lang.uom_pcs') }}</option>
                            <option value="cm" {{ old('unit_of_measure', $trim->unit_of_measure ?? '') === 'cm' ? 'selected' : '' }}>{{ __('projectx::lang.uom_cm') }}</option>
                            <option value="inches" {{ old('unit_of_measure', $trim->unit_of_measure ?? '') === 'inches' ? 'selected' : '' }}>{{ __('projectx::lang.uom_inches') }}</option>
                            <option value="yards" {{ old('unit_of_measure', $trim->unit_of_measure ?? '') === 'yards' ? 'selected' : '' }}>{{ __('projectx::lang.uom_yards') }}</option>
                            <option value="sets" {{ old('unit_of_measure', $trim->unit_of_measure ?? '') === 'sets' ? 'selected' : '' }}>{{ __('projectx::lang.uom_sets') }}</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-lg-9">
                        <input type="number" min="{{ $projectxZeroMin }}" step="{{ $projectxQuantityStep }}" name="quantity_per_garment" class="form-control form-control-solid" value="{{ old('quantity_per_garment', $trim->quantity_per_garment ?? '') }}" />
                    </div>
                </div>
                    ...
                    <div class="col-lg-9">
                        <select name="supplier_contact_id" class="form-select form-select-solid" data-control="select2">
                            ...
                            @foreach(($suppliers ?? []) as $supplierKey => $supplierItem)
                                <option
                                    value="{{ data_get($supplierItem, 'id', $supplierKey) }}"
                                    {{ (string) old('supplier_contact_id', $trim->supplier_contact_id ?? '') === (string) data_get($supplierItem, 'id', $supplierKey) ? 'selected' : '' }}
                                >
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-lg-9">
                        <div class="input-group">
                            ...
                            <input type="number" min="{{ $projectxZeroMin }}" step="{{ $projectxCurrencyStep }}" name="unit_cost" class="form-control form-control-solid" value="{{ old('unit_cost', $trim->unit_cost ?? '') }}" />
                        </div>
                    </div>
                </div>
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-lg-9">
                        <input type="text" name="currency" class="form-control form-control-solid" value="{{ old('currency', $trim->currency ?? data_get($currency ?? [], 'code', 'USD')) }}" />
                    </div>
                </div>
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-lg-9">
                        <input type="number" min="0" step="1" name="lead_time_days" class="form-control form-control-solid" value="{{ old('lead_time_days', $trim->lead_time_days ?? '') }}" />
                    </div>
                </div>
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-lg-9">
                        <select name="status" class="form-select form-select-solid" data-control="select2" data-hide-search="true">
                            <option value="draft" {{ old('status', $trim->status ?? 'draft') === 'draft' ? 'selected' : '' }}>{{ __('projectx::lang.trim_status_draft') }}</option>
                            <option value="sample_requested" {{ old('status', $trim->status ?? '') === 'sample_requested' ? 'selected' : '' }}>{{ __('projectx::lang.trim_status_sample_requested') }}</option>
                            <option value="sample_received" {{ old('status', $trim->status ?? '') === 'sample_received' ? 'selected' : '' }}>{{ __('projectx::lang.trim_status_sample_received') }}</option>
                            <option value="approved" {{ old('status', $trim->status ?? '') === 'approved' ? 'selected' : '' }}>{{ __('projectx::lang.trim_status_approved') }}</option>
                            <option value="bulk_ordered" {{ old('status', $trim->status ?? '') === 'bulk_ordered' ? 'selected' : '' }}>{{ __('projectx::lang.trim_status_bulk_ordered') }}</option>
                            <option value="bulk_received" {{ old('status', $trim->status ?? '') === 'bulk_received' ? 'selected' : '' }}>{{ __('projectx::lang.trim_status_bulk_received') }}</option>
                            <option value="qc_passed" {{ old('status', $trim->status ?? '') === 'qc_passed' ? 'selected' : '' }}>{{ __('projectx::lang.trim_status_qc_passed') }}</option>
                            <option value="qc_failed" {{ old('status', $trim->status ?? '') === 'qc_failed' ? 'selected' : '' }}>{{ __('projectx::lang.trim_status_qc_failed') }}</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-lg-9">
                        <input
                            type="text"
                            id="kt_projectx_trim_edit_qc_at"
                            name="qc_at"
                            class="form-control form-control-solid"
                            value="{{ old('qc_at', !empty($trim->qc_at) ? \Illuminate\Support\Carbon::parse($trim->qc_at)->format('Y-m-d\\TH:i') : '') }}"
                            placeholder="{{ __('projectx::lang.qc_at') }}"
                        />
                    </div>
                </div>
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-lg-9">
                        <textarea name="qc_notes" class="form-control form-control-solid" rows="3">{{ old('qc_notes', $trim->qc_notes ?? '') }}</textarea>
                    </div>
                </div>
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-lg-9">
                        <textarea name="care_testing" class="form-control form-control-solid" rows="3">{{ old('care_testing', $trim->care_testing ?? '') }}</textarea>
                    </div>
                </div>
...
@section('page_javascript')
    <script>
        (function () {
            const qcAtInput = document.getElementById('kt_projectx_trim_edit_qc_at');
