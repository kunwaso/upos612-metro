...
@section('title', __('projectx::lang.quote_detail'))
...
<div class="d-flex flex-wrap flex-stack mb-6">
    <div>
        <h1 class="text-gray-900 fw-bold mb-1">{{ __('projectx::lang.quote_detail') }}</h1>
        <div class="text-muted fw-semibold fs-6">{{ __('projectx::lang.quote_no') }}: {{ $quoteDisplay['quoteNumber'] ?? ($quote->quote_number ?: $quote->uuid) }}</div>
    </div>
    <div class="d-flex gap-2">
        ...
        @if($quoteActionFlags['showEdit'] ?? false)
            <a href="{{ route('projectx.quotes.edit', ['id' => $quote->id]) }}" class="btn btn-light-info btn-sm">
        ...
        @if($quoteActionFlags['canCreateSaleFromQuote'] ?? false)
            <a href="{{ route('sells.create', ['projectx_quote_id' => $quote->id]) }}" class="btn btn-primary btn-sm">
                ...
                {{ __('projectx::lang.create_sale_from_quote') }}
            </a>
        @endif
        @if($quote->transaction_id)
            <a href="{{ route('projectx.sales.orders.show', ['id' => $quote->transaction_id]) }}" class="btn btn-light-success btn-sm">
                <i class="ki-duotone ki-document fs-5 me-1"><span class="path1"></span><span class="path2"></span></i>
                {{ __('projectx::lang.view_order') }}
            </a>
        @endif
    </div>
</div>
    ...
    <div class="col-xl-4">
        <div class="card card-flush mb-5">
            <div class="card-header pt-7">
                <h3 class="card-title fw-bold text-gray-900">{{ __('projectx::lang.quote_summary') }}</h3>
            </div>
            <div class="card-body pt-5">
                ...
                <div class="d-flex flex-stack mb-4">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.customer') }}</span>
                    <span class="text-gray-900 fw-bold">{{ $quoteDisplay['customerName'] ?? '-' }}</span>
                </div>
                <div class="d-flex flex-stack mb-4">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.customer_email') }}</span>
                    <span class="text-gray-900 fw-bold">{{ $quoteDisplay['customerEmail'] ?? '-' }}</span>
                </div>
                <div class="d-flex flex-stack mb-4">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.location') }}</span>
                    <span class="text-gray-900 fw-bold">{{ $quoteDisplay['locationName'] ?? '-' }}</span>
                </div>
                <div class="d-flex flex-stack mb-4">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.status') }}</span>
                    <span><span class="badge {{ $quoteDisplay['quoteStateBadgeClass'] ?? 'badge-light-secondary' }}">{{ $quoteDisplay['quoteStateLabel'] ?? __('projectx::lang.quote_state_draft') }}</span></span>
                </div>
                <div class="d-flex flex-stack mb-4">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.lines') }}</span>
                    <span class="text-gray-900 fw-bold">{{ $quoteDisplay['quoteLineCount'] ?? 0 }}</span>
                </div>
                <div class="d-flex flex-stack mb-4">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.currency_label') }}</span>
                    <span class="text-gray-900 fw-bold">{{ $quoteDisplay['currencyCode'] ?? '-' }}</span>
                </div>
                <div class="d-flex flex-stack mb-4">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.incoterm') }}</span>
                    <span class="text-gray-900 fw-bold">{{ $quote->incoterm ?: '-' }}</span>
                </div>
                <div class="d-flex flex-stack mb-4">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.quote_date') }}</span>
                    <span class="text-gray-900 fw-bold">{{ $quoteDisplay['quoteDateDisplay'] ?? '-' }}</span>
                </div>
                <div class="d-flex flex-stack mb-4">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.valid_until') }}</span>
                    <span class="text-gray-900 fw-bold">{{ $quoteDisplay['validUntilDisplay'] ?? '-' }}</span>
                </div>
                @if($quote->shipment_port)
                <div class="d-flex flex-stack mb-4">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.shipment_port') }}</span>
                    <span class="text-gray-900 fw-bold">{{ $quote->shipment_port }}</span>
                </div>
                @endif
                <div class="d-flex flex-stack mb-6">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.grand_total') }}</span>
                ...
                @if($quoteActionFlags['showOverrideNotice'] ?? false)
                    <div class="alert alert-warning d-flex align-items-start mb-4">
                        ...
                        <div class="fw-semibold">{{ __('projectx::lang.quote_admin_override_notice') }}</div>
                    </div>
                @endif

                @if($quoteActionFlags['canSendQuote'] ?? false)
                    <form method="POST" action="{{ route('projectx.quotes.send', ['id' => $quote->id]) }}" class="mb-4">
                        @csrf
                        <label class="form-label">{{ __('projectx::lang.send_to_email') }}</label>
                        <div class="input-group">
                            ...
                            <button type="submit" class="btn btn-primary">{{ __('projectx::lang.send_quote') }}</button>
                        </div>
                    </form>
                @endif

                @if($quoteActionFlags['showDelete'] ?? false)
                    <form method="POST" action="{{ route('projectx.quotes.destroy', ['id' => $quote->id]) }}" class="mb-4" onsubmit="return confirm('{{ __('projectx::lang.delete_quote_confirm') }}');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-light-danger w-100">{{ __('projectx::lang.delete') }}</button>
                    </form>
                @endif

                @if($quoteActionFlags['showRevertToDraft'] ?? false)
                    <form method="POST" action="{{ route('projectx.quotes.revert_draft', ['id' => $quote->id]) }}" class="mb-4" onsubmit="return confirm('{{ __('projectx::lang.revert_quote_to_draft_confirm') }}');">
                        @csrf
                        <button type="submit" class="btn btn-light-warning w-100">{{ __('projectx::lang.revert_quote_to_draft') }}</button>
                    </form>
                @endif

                @if($quoteActionFlags['showClearSignature'] ?? false)
                    <form method="POST" action="{{ route('projectx.quotes.clear_signature', ['id' => $quote->id]) }}" class="mb-4" onsubmit="return confirm('{{ __('projectx::lang.clear_quote_signature_confirm') }}');">
                        @csrf
                        <button type="submit" class="btn btn-light-danger w-100">{{ __('projectx::lang.clear_quote_signature') }}</button>
                    </form>
                @endif

                <div class="d-grid gap-2 mb-6">
                    <a href="{{ $publicUrl }}" target="_blank" class="btn btn-light-primary">{{ __('projectx::lang.open_public_quote') }}</a>
                ...
                <div class="mb-3 d-flex flex-stack">
                    <span class="text-gray-500 fw-semibold">{{ __('projectx::lang.public_link_password') }}</span>
                    @if(!empty($quote->public_link_password))
                        <span class="badge badge-light-success">{{ __('projectx::lang.password_set') }}</span>
                    @else
                        <span class="badge badge-light-secondary">{{ __('projectx::lang.password_not_set') }}</span>
                    @endif
                </div>

                <form method="POST" action="{{ route('projectx.quotes.set_public_password', ['id' => $quote->id]) }}" class="mb-4">
                    @csrf
                    <div class="mb-3">
                    ...
                    <div class="mb-4">
                        <label class="form-label">{{ __('projectx::lang.password_confirmation') }}</label>
                        <input type="password" name="password_confirmation" class="form-control form-control-solid" autocomplete="new-password">
                        @error('password_confirmation')
                            <div class="text-danger fs-7 mt-2">{{ $message }}</div>
                        @enderror
                    </div>
                    <button type="submit" class="btn btn-primary w-100">{{ __('projectx::lang.set_password') }}</button>
                </form>

                <form method="POST" action="{{ route('projectx.quotes.set_public_password', ['id' => $quote->id]) }}">
                    @csrf
                    <input type="hidden" name="remove_password" value="1">
                    <button type="submit" class="btn btn-light-danger w-100">{{ __('projectx::lang.remove_password') }}</button>
                </form>
            </div>
        </div>
    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center text-muted">{{ __('projectx::lang.no_records_found') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                        <tfoot>
