...
@section('title', __('projectx::lang.edit_quote'))

@section('content')
<div class="d-none" data-projectx-quote-id="{{ (int) $quote->id }}"></div>
<div class="d-flex flex-wrap flex-stack mb-6">
    <div>
        <h1 class="text-gray-900 fw-bold mb-1">{{ __('projectx::lang.edit_quote') }}</h1>
        <div class="text-muted fw-semibold fs-6">{{ __('projectx::lang.quote_no') }}: {{ $quote->quote_number ?: $quote->uuid }}</div>
    </div>
    <a href="{{ route('projectx.quotes.show', ['id' => $quote->id]) }}" class="btn btn-light-primary btn-sm">
        <i class="ki-duotone ki-arrow-left fs-5 me-1"><span class="path1"></span><span class="path2"></span></i>
        {{ __('projectx::lang.view_quote') }}
    </a>
</div>

<form method="POST" action="{{ route('projectx.quotes.update', ['id' => $quote->id]) }}" id="projectx_quote_edit_form">
    @csrf
    @method('PATCH')
        ...
        <div class="card-body pt-5">
            <div class="row g-5">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control form-control-solid" id="kt_projectx_quote_edit_quote_date" name="quote_date" value="{{ old('quote_date', optional($quote->quote_date)->format('Y-m-d') ?? $quote->created_at->format('Y-m-d')) }}" placeholder="{{ __('projectx::lang.quote_date') }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label required">{{ __('projectx::lang.valid_until') }}</label>
                    <input type="text" class="form-control form-control-solid" id="kt_projectx_quote_edit_expires_at" name="expires_at" value="{{ old('expires_at', optional($quote->expires_at)->format('Y-m-d')) }}" placeholder="{{ __('projectx::lang.valid_until') }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">{{ __('projectx::lang.shipment_port') }}</label>
                    <input type="text" class="form-control form-control-solid" name="shipment_port" list="shipment_port_list" value="{{ old('shipment_port', $quote->shipment_port) }}" placeholder="{{ __('projectx::lang.shipment_port') }}">
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control form-control-solid" name="remark" rows="6" placeholder="{{ __('projectx::lang.remark') }}">{{ old('remark', $quote->remark ?? '') }}</textarea>
                </div>
            </div>
        </div>
    </div>
