...
use Modules\ProjectX\Http\Requests\Essentials\EssentialsHrmAttendanceImportRequest;
use Modules\ProjectX\Http\Requests\Essentials\EssentialsHrmAttendanceStoreRequest;
use Modules\ProjectX\Http\Requests\Essentials\EssentialsHrmAttendanceUpdateRequest;
use Modules\ProjectX\Http\Requests\Essentials\EssentialsHrmClockInOutRequest;
use Modules\ProjectX\Utils\ProjectXEssentialsUtil;
use Spatie\Permission\Models\Permission;
use Yajra\DataTables\Facades\DataTables;

class ProjectXEssentialsAttendanceController extends Controller
{
    /**
     * All Utils instance.
     */
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($attendance)
                    ->addColumn(
                        'action',
                        '@can("essentials.crud_all_attendance") <button data-href="{{route(\'projectx.essentials.hrm.attendance.edit\', [\'attendance\' => $id])}}" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-primary btn-modal" data-container="#edit_attendance_modal"><i class="glyphicon glyphicon-edit"></i> @lang("messages.edit")</button>
                        <button class="tw-dw-btn tw-dw-btn-outline tw-dw-btn-xs tw-dw-btn-error delete-attendance" data-href="{{route(\'projectx.essentials.hrm.attendance.destroy\', [\'attendance\' => $id])}}"><i class="fa fa-trash"></i> @lang("messages.delete")</button> @endcan
                        '
                    )
        ...
        return view('projectx::essentials.hrm.attendance.index')
    ...
    public function create()
    {
        ...
        return view('projectx::essentials.hrm.attendance.create')->with(compact('employees'));
    }
     ...
     * @return Response
     */
    public function store(EssentialsHrmAttendanceStoreRequest $request)
    {
        $business_id = $request->session()->get('user.business_id');
        $is_admin = $this->moduleUtil->is_admin(auth()->user(), $business_id);
    ...
    public function edit($id)
    {
        ...
        return view('projectx::essentials.hrm.attendance.edit')->with(compact('attendance'));
    }
     ...
     * @return Response
     */
    public function update(EssentialsHrmAttendanceUpdateRequest $request, $id)
    {
        $business_id = $request->session()->get('user.business_id');
        $is_admin = $this->moduleUtil->is_admin(auth()->user(), $business_id);
     ...
     * @return Response
     */
    public function clockInClockOut(EssentialsHrmClockInOutRequest $request)
    {
        $business_id = $request->session()->get('user.business_id');
        ...
        if (! auth()->user()->can('essentials.allow_users_for_attendance_from_web')) {
            return $this->respondWithError(__('essentials::lang.not_allowed'));
        } elseif ((! empty($settings['is_location_required']) && $settings['is_location_required']) && empty($request->input('clock_in_out_location'))) {
            return $this->respondWithError(__('essentials::lang.you_must_enable_location'));
        }
    ...
    public function getAttendanceByShift()
    {
        ...
        foreach ($attendance_by_shift as $shift_id => $summary) {
            $attendance_by_shift[$shift_id]['absent_users'] = array_values(array_diff(
                $summary['all_users'],
                $summary['present_users']
            ));
        }

        return view('projectx::essentials.hrm.attendance.attendance_by_shift_data')->with(compact('attendance_by_shift'));
    }
    ...
    public function getAttendanceByDate()
    {
        ...
        return view('projectx::essentials.hrm.attendance.attendance_by_date_data')->with(compact('attendance_by_date'));
    }
     ...
     * @return Response
     */
    public function importAttendance(EssentialsHrmAttendanceImportRequest $request)
    {
        $business_id = request()->session()->get('user.business_id');
        $is_admin = $this->moduleUtil->is_admin(auth()->user(), $business_id);
    ...
    public function getAttendanceRow($user_id)
    {
        ...
        return view('projectx::essentials.hrm.attendance.attendance_row')->with(compact('attendance', 'shifts', 'user'));
    }
}
