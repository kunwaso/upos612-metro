...
use Modules\ProjectX\Http\Requests\Essentials\EssentialsHrmPayrollAddPaymentRequest;
use Modules\ProjectX\Http\Requests\Essentials\EssentialsHrmPayrollGroupUpdateRequest;
use Modules\ProjectX\Http\Requests\Essentials\EssentialsHrmPayrollStoreRequest;
use Modules\ProjectX\Http\Requests\Essentials\EssentialsHrmPayrollUpdateRequest;
use Modules\ProjectX\Utils\ProjectXEssentialsUtil;
use Yajra\DataTables\Facades\DataTables;

class ProjectXEssentialsPayrollController extends Controller
{
    /**
     * All Utils instance.
     */
    ...
    protected $projectXEssentialsUtil;
    ...
    public function index()
    {
            ...
            return Datatables::of($payrolls)
                ->addColumn(
                    ...
                    function ($row) {
                        ...
                        $html .= '<li><a href="#" data-href="'.route('projectx.essentials.hrm.payroll.show', ['payroll' => $row->id]).'" data-container=".view_modal" class="btn-modal"><i class="fa fa-eye" aria-hidden="true"></i> '.__('messages.view').'</a></li>';
        ...
        return view('projectx::essentials.hrm.payroll.index')->with(compact('employees', 'departments', 'designations', 'locations'));
    }
    ...
    public function create()
    {
        ...
        if (! empty($add_payroll_for)) {
            ...
            $view_data = $this->preparePayrollCreateFormViewData([
                'action' => 'create',
            ...
            return view('projectx::essentials.hrm.payroll.create')->with($view_data);
        } else {
            return redirect()->route('projectx.essentials.hrm.payroll.index')
                ->with('status',
                    [
                        'success' => true,
     ...
     * @return Response
     */
    public function store(EssentialsHrmPayrollStoreRequest $request)
    {
        $business_id = request()->session()->get('user.business_id');
        ...
        return redirect()->route('projectx.essentials.hrm.payroll.index')->with('status', $output);
    }
    ...
    public function show($id)
    {
        ...
        foreach ($payroll->payment_lines as $payment_line) {
            $payment_method_labels[$payment_line->id] = ! empty($payment_types[$payment_line->method])
                ? $payment_types[$payment_line->method]
                : $payment_line->method;
        }
        $department_name = ! empty($department) ? $department->name : '';
        $designation_name = ! empty($designation) ? $designation->name : '';
        $location_display_name = ! empty($location) ? $location->name : __('report.all_locations');
        ...
        $pay_component_rows = $this->preparePayrollShowRows($allowances, $deductions);

        return view('projectx::essentials.hrm.payroll.show')
        ...
        'total_work_duration', 'location', 'total_days_present', 'pay_component_rows', 'department_name',
        'designation_name', 'location_display_name', 'payment_method_labels'));
    }
    ...
    public function edit($id)
    {
        ...
        $allowance_rows = $this->preparePayrollComponentRows($allowances, 'allowance');
        $deduction_rows = $this->preparePayrollComponentRows($deductions, 'deduction');
        $total_amount = $payroll->essentials_duration * $payroll->essentials_amount_per_unit_duration;

        return view('projectx::essentials.hrm.payroll.edit')
            ->with(compact('payroll', 'month_name', 'allowances', 'deductions', 'year', 'allowance_rows', 'deduction_rows', 'total_amount'));
    }
     ...
     * @return Response
     */
    public function update(EssentialsHrmPayrollUpdateRequest $request, $id)
    {
        $business_id = request()->session()->get('user.business_id');
    ...
    public function getAllowanceAndDeductionRow(Request $request)
    {
        if ($request->ajax()) {
            ...
            $view_data = $this->prepareAllowanceDeductionRowViewData((string) $type, (string) $employee);

            $ad_row = view('projectx::essentials.hrm.payroll.allowance_and_deduction_row')
                        ->with($view_data)
                        ->render();
            ...
            return Datatables::of($payroll_groups)
                ->addColumn(
                    ...
                    function ($row) {
                        ...
                        $html .= '<li>
                                    <a href="'.route('projectx.essentials.hrm.payroll.view-payroll-group', ['id' => $row->id]).'" target="_blank">
                        ...
                        if (auth()->user()->can('essentials.update_payroll')) {
                            $html .= '<li>
                                        <a href="'.route('projectx.essentials.hrm.payroll.edit-payroll-group', ['id' => $row->id]).'" target="_blank">
                        ...
                        if (auth()->user()->can('essentials.delete_payroll') && $row->status == 'draft') {
                            $html .= '<li><a href="'.route('projectx.essentials.hrm.payroll.destroy', ['payroll' => $row->id]).'" class="delete-payroll"><i class="fa fa-trash" aria-hidden="true"></i> '.__('messages.delete').'</a></li>';
                        }

                        if ($row->status == 'final' && $row->payment_status != 'paid') {
                            $html .= '<li>
                                    <a href="'.route('projectx.essentials.hrm.payroll.add-payment', ['id' => $row->id]).'" target="_blank">
    ...
    public function viewPayrollGroup($id)
    {
        ...
        foreach ($payroll_group->payrollGroupTransactions as $transaction) {
            ...
            $payrolls[$transaction->expense_for]['bank_name'] = ! empty($bank_details['bank_name']) ? $bank_details['bank_name'] : '';
            $payrolls[$transaction->expense_for]['bank_account_holder_name'] = ! empty($bank_details['account_holder_name']) ? $bank_details['account_holder_name'] : '';
            $payrolls[$transaction->expense_for]['bank_account_number'] = ! empty($bank_details['account_number']) ? $bank_details['account_number'] : '';
        }

        return view('projectx::essentials.hrm.payroll.view_payroll_group')
    ...
    public function getUpdatePayrollGroup(EssentialsHrmPayrollGroupUpdateRequest $request)
    {
        $business_id = request()->session()->get('user.business_id');
        if (! (auth()->user()->can('superadmin') || auth()->user()->can('essentials.update_payroll') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module'))) {
            abort(403, 'Unauthorized action.');
    ...
    public function addPayment($id)
    {
        ...
        return view('projectx::essentials.hrm.payroll.pay_payroll_group')
            ->with(compact('payroll_group', 'month_name', 'year', 'payrolls', 'payment_types', 'accounts'));
    }

    public function postAddPayment(EssentialsHrmPayrollAddPaymentRequest $request)
    {
        $business_id = request()->session()->get('user.business_id');
        $is_admin = $this->moduleUtil->is_admin(auth()->user(), $business_id);
        if (! (auth()->user()->can('superadmin') || $is_admin || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module'))) {
    ...
    public function getMyPayrolls(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($payrolls)
                ->addColumn(
                    'action',
                    function ($row) {
                        $html = '<a href="#" data-href="'.route('projectx.essentials.hrm.payroll.show', ['payroll' => $row->id]).'" data-container=".view_modal" class="btn-modal btn-info btn btn-sm">
                            <i class="fa fa-eye" aria-hidden="true"></i> '
                            .__('messages.view').
                            '</a>';
        ...
        return view('projectx::essentials.hrm.payroll.partials.user_payrolls')
     ...
     */
    protected function preparePayrollCreateFormViewData(array $data): array
    {
        $is_edit = ($data['action'] ?? 'create') === 'edit';
        $month_name = (string) ($data['month_name'] ?? '');
        $year = (string) ($data['year'] ?? '');
        $transaction_date_input = $data['transaction_date'] ?? '';
        $transaction_date = $transaction_date_input instanceof \Carbon\CarbonInterface
            ? $transaction_date_input->format('Y-m-d')
            : (string) $transaction_date_input;
        $location = $data['location'] ?? null;
        $payroll_group = $data['payroll_group'] ?? null;
        $payrolls = (array) ($data['payrolls'] ?? []);

        $group_name = __('essentials::lang.payroll_for_month', ['date' => $month_name . ' ' . $year]);

        return [
            'is_edit' => $is_edit,
            'title' => $is_edit ? __('essentials::lang.edit_payroll') : __('essentials::lang.add_payroll'),
            'form_action' => $is_edit
                ? route('projectx.essentials.hrm.payroll.update-payroll-group')
                : route('projectx.essentials.hrm.payroll.store'),
            'submit_label' => $is_edit ? __('messages.update') : __('messages.save'),
            'transaction_date' => $transaction_date,
            'payroll_group_id' => $is_edit && ! empty($payroll_group) ? $payroll_group->id : null,
            'payroll_group_name_value' => $is_edit && ! empty($payroll_group)
                ? $payroll_group->name
                : strip_tags($group_name),
            'payroll_group_status_value' => $is_edit && ! empty($payroll_group)
                ? $payroll_group->status
                : 'draft',
            'location_id' => ! empty($location) ? $location->id : '',
            'location_name' => ! empty($location) ? $location->name : __('report.all_locations'),
            'group_name' => $group_name,
            'payroll_rows' => $this->preparePayrollRowsForForm($payrolls, $transaction_date),
        ];
    }

    /**
     * @param  array<int|string, array<string, mixed>>  $payrolls
     * @return array<int, array<string, mixed>>
     */
    protected function preparePayrollRowsForForm(array $payrolls, string $transaction_date): array
    {
        $rows = [];
        $days_in_month = ! empty($transaction_date) ? \Carbon::parse($transaction_date)->daysInMonth : 30;

        foreach ($payrolls as $employee_id => $payroll) {
            $pay_period = (string) ($payroll['essentials_pay_period'] ?? '');
            $duration_default = $pay_period === 'week'
                ? 4
                : ($pay_period === 'day' ? $days_in_month : 1);
            $duration_unit_default = $pay_period === 'week'
                ? __('essentials::lang.week')
                : ($pay_period === 'day' ? __('lang_v1.day') : __('lang_v1.month'));

            $rows[] = [
                'employee_id' => (int) $employee_id,
                'name' => $payroll['name'] ?? '',
                'transaction_id' => $payroll['transaction_id'] ?? null,
                'essentials_duration' => $payroll['essentials_duration'] ?? $duration_default,
                'essentials_duration_unit' => $payroll['essentials_duration_unit'] ?? $duration_unit_default,
                'essentials_amount_per_unit_duration' => $payroll['essentials_amount_per_unit_duration'] ?? ($payroll['essentials_salary'] ?? 0),
                'staff_note' => $payroll['staff_note'] ?? '',
                'allowance_rows' => $this->preparePayrollComponentRows((array) ($payroll['allowances'] ?? []), 'allowance'),
                'deduction_rows' => $this->preparePayrollComponentRows((array) ($payroll['deductions'] ?? []), 'deduction'),
            ];
        }
    ...
    /**
     * @param  array<string, mixed>  $component_data
     * @return array<int, array<string, mixed>>
     */
    protected function preparePayrollComponentRows(array $component_data, string $type): array
    {
        $name_key = $type === 'allowance' ? 'allowance_names' : 'deduction_names';
        $type_key = $type === 'allowance' ? 'allowance_types' : 'deduction_types';
        $amount_key = $type === 'allowance' ? 'allowance_amounts' : 'deduction_amounts';
        $percent_key = $type === 'allowance' ? 'allowance_percents' : 'deduction_percents';

        $names = array_values((array) ($component_data[$name_key] ?? []));
        $types = array_values((array) ($component_data[$type_key] ?? []));
        $amounts = array_values((array) ($component_data[$amount_key] ?? []));
        $percents = array_values((array) ($component_data[$percent_key] ?? []));
        ...
        $row_count = max(count($names), count($types), count($amounts), count($percents), 1);
        $rows = [];
        for ($index = 0; $index < $row_count; $index++) {
            $row_type = $types[$index] ?? 'fixed';
            $rows[] = [
                'name' => $names[$index] ?? '',
                'type' => $row_type,
                'amount' => $amounts[$index] ?? 0,
                'percent' => $percents[$index] ?? 0,
                'is_percent' => $row_type === 'percent',
            ];
        }
    ...
    /**
     * @param  array<string, mixed>  $allowances
     * @param  array<string, mixed>  $deductions
     * @return array<int, array<string, mixed>>
     */
    protected function preparePayrollShowRows(array $allowances, array $deductions): array
    {
        $allowance_names = array_values((array) ($allowances['allowance_names'] ?? []));
        $allowance_amounts = array_values((array) ($allowances['allowance_amounts'] ?? []));
        $deduction_names = array_values((array) ($deductions['deduction_names'] ?? []));
        $deduction_amounts = array_values((array) ($deductions['deduction_amounts'] ?? []));

        $row_count = max(count($allowance_names), count($allowance_amounts), count($deduction_names), count($deduction_amounts), 1);
        $rows = [];
        for ($index = 0; $index < $row_count; $index++) {
            $rows[] = [
                'allowance_name' => $allowance_names[$index] ?? '',
                'allowance_amount' => $allowance_amounts[$index] ?? null,
                'deduction_name' => $deduction_names[$index] ?? '',
                'deduction_amount' => $deduction_amounts[$index] ?? null,
            ];
        }
    ...
    public function getEmployeesBasedOnLocation(Request $request)
    {
        ...
        try {
            ...
            $employees_html = view('projectx::essentials.hrm.payroll.partials.employee_dropdown')
     ...
     */
    protected function prepareAllowanceDeductionRowViewData(string $type, string $employee): array
    {
        $is_allowance = $type === 'allowance';

        return [
            'employee' => $employee,
            'name_col' => $is_allowance ? 'allowance_names' : 'deduction_names',
            'type_col' => $is_allowance ? 'allowance_types' : 'deduction_types',
            'percent_col' => $is_allowance ? 'allowance_percent' : 'deduction_percent',
            'amount_col' => $is_allowance ? 'allowance_amounts' : 'deduction_amounts',
            'amount_class' => $is_allowance ? 'allowance' : 'deduction',
            'button_class' => $is_allowance ? 'add_allowance' : 'add_deduction',
        ];
    }
