...
use Modules\ProjectX\Http\Requests\ConfirmPublicQuoteRequest;
use Modules\ProjectX\Http\Requests\UnlockPublicQuoteRequest;
...
class PublicQuoteController extends Controller
{
    ...
    public function __construct(QuoteMailerInterface $quoteMailer, ProjectXQuoteDisplayPresenter $quoteDisplayPresenter)
    {
    ...
    public function show(Request $request, string $publicToken)
    {
        ...
        if ($this->isPasswordProtected($quote) && ! $this->isQuoteUnlocked($publicToken, $quote)) {
            ...
            return response()
                ->view('projectx::quotes.public_quote_password', [
                    'publicToken' => $publicToken,
                    'locked' => $locked,
                    'lockedMessage' => $locked ? __('projectx::lang.quote_unlock_blocked_10min') : null,
                ])
        ...
        return response()
            ->view('projectx::quotes.public_quote', compact('quote', 'quoteDisplay'))
            ->header('X-Robots-Tag', 'noindex, nofollow');
    }
    ...
    public function confirm(ConfirmPublicQuoteRequest $request, string $publicToken)
    {
        ...
        if ($this->isPasswordProtected($quote) && ! $this->isQuoteUnlocked($publicToken, $quote)) {
            return redirect()
                ->route('projectx.quotes.public', ['publicToken' => $publicToken])
                ->with('status', ['success' => false, 'msg' => __('projectx::lang.enter_password_to_view_quote')]);
        }

        if ($quote->confirmed_at) {
            return redirect()
                ->route('projectx.quotes.public', ['publicToken' => $publicToken])
                ->with('status', ['success' => false, 'msg' => __('projectx::lang.quote_already_confirmed')]);
        }
        ...
        if (! empty(optional($quote->creator)->email)) {
            try {
                ...
                \Log::warning('ProjectX quote confirmation mail failed', [
                    'quote_id' => $quote->id,
        ...
        return redirect()
            ->route('projectx.quotes.public', ['publicToken' => $publicToken])
            ->with('status', ['success' => true, 'msg' => __('projectx::lang.quote_confirmed_success')]);
    }
    ...
    public function unlock(UnlockPublicQuoteRequest $request, string $publicToken)
    {
        ...
        if (! $this->isPasswordProtected($quote)) {
            return redirect()->route('projectx.quotes.public', ['publicToken' => $publicToken]);
        }
        ...
        if ($this->isQuoteUnlockBlocked($publicToken, $request->ip())) {
            return redirect()
                ->route('projectx.quotes.public', ['publicToken' => $publicToken])
                ->withErrors([
                    'password' => __('projectx::lang.quote_unlock_blocked_10min'),
                ]);
        }
        ...
        if (! Hash::check((string) $request->input('password'), (string) $quote->public_link_password)) {
            ...
            $message = $attempts >= self::MAX_FAILED_ATTEMPTS
                ? __('projectx::lang.quote_too_many_failed_attempts')
                : __('projectx::lang.invalid_quote_password');
    ...
    {
        return 'projectx_quote_unlocked_token_' . $publicToken;
    }

    protected function passwordFingerprint(Quote $quote): string
    {
        return hash('sha256', (string) $quote->public_link_password);
    }

    protected function isPasswordProtected(Quote $quote): bool
    {
        return ! empty($quote->public_link_password);
    }

    protected function isQuoteUnlocked(string $publicToken, Quote $quote): bool
    {
        if (! $this->isPasswordProtected($quote)) {
            return true;
        }
    ...
    protected function markQuoteUnlocked(string $publicToken, Quote $quote): void
    {
        if (! $this->isPasswordProtected($quote)) {
    ...
    protected function findQuoteByPublicToken(string $publicToken): ?Quote
    {
        return Quote::with([
    ...
    {
        return 'projectx.quote_unlock_attempts.' . $publicToken . '.' . sha1((string) ($ipAddress ?: 'unknown'));
    }

    protected function passwordCooldownKey(string $publicToken, ?string $ipAddress): string
    {
        return 'projectx.quote_unlock_cooldown.' . $publicToken . '.' . sha1((string) ($ipAddress ?: 'unknown'));
    }
